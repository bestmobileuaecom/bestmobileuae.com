...
export async function getPhoneBySlug(slug, includePreview = false) {
  ...
  return {
    ...
    storePrices: storePrices?.map((sp) => ({
      ...
      url: sp.url,
      region: sp.region,
      logo_url: sp.stores?.logo_url || null,
    })) || [],
    verdict: phone.verdict,
    // Auto-calculate 1-5 scores from 0-10 scores (divide by 2)
    // Read from detailed_scores first (has decimals), fallback to integer fields
    scores: (() => {
      const getScore = (label) => {
        const detailed = phone.detailed_scores?.find(s => s.label === label);
        return detailed?.rating || 0;
      };
      return [
        { label: "Value", icon: "💰", rating: getScore("Value") / 2 },
        { label: "Daily Use", icon: "📱", rating: getScore("Performance") / 2 },
        { label: "Camera", icon: "📸", rating: getScore("Camera") / 2 },
        { label: "Gaming", icon: "🎮", rating: getScore("Performance") / 2 },
        { label: "Battery", icon: "🔋", rating: getScore("Battery") / 2 },
      ];
    })(),
